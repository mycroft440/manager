...
def gerenciar_zram():
    """Menu para gerenciar o ZRAM."""
    while True:
        print("\n--- Gerenciar ZRAM ---")
        print("1. Configurar/Ativar ZRAM")
        print("2. Desativar/Remover ZRAM")
        print("3. Instalar Serviço Systemd (Auto-iniciar)")
        print("4. Desinstalar Serviço Systemd")
        print("0. Voltar")
        ...
        if choice == '1':
            try:
                percentage = float(input("Porcentagem da RAM para ZRAM (ex: 0.3 para 30%): ") or 0.3)
                algorithm = input("Algoritmo de compressão (zstd, lz4, lzo - padrão: zstd): ") or 'zstd'
                priority = int(input("Prioridade do swap (padrão: 100): ") or 100)
                num_devices = input("Número de dispositivos ZRAM (padrão: CPU cores): ")
                num_devices = int(num_devices) if num_devices.isdigit() else None
                setup_zram(percentage, algorithm, priority, num_devices)
            except ValueError as e:
                print(f"Erro: {e}")
        elif choice == '2':
            teardown_zram()
        elif choice == '3':
            install_systemd_service(os.path.abspath(__file__))
        elif choice == '4':
            uninstall_systemd_service()
        elif choice == '0':
            break
